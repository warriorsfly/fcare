...
class TimesUtils {
    ...
    companion object {
        ...
        fun selectTime(contect:Context,selectTimeListener: SelectTimeListener,type:String){
            ...
            var dialog=DatePickerDialog(contect, DatePickerDialog.OnDateSetListener { view, year, monthOfYear, dayOfMonth ->
                mYear = year
                mMonth = monthOfYear
                mDay = dayOfMonth
                val timeDialog=TimePickerDialog(contect,
                    TimePickerDialog.OnTimeSetListener{ timePicker: TimePicker, i: Int, i1: Int ->
                        mHour = i
                        mMin = i1
                        var times = StringBuffer().append(mYear).append("-").append(mMonth + 1).append("-").append(mDay).append(" ")
